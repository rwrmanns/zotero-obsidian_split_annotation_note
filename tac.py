...
do_QA = {
    ...
    "QA_d8_hash"     : QA_d8_hash,       # hash of specific QA QA_deck included
...
def get_lo_QA_tag_block(text):
    # QA_tag_block == block of text ...
...
def get_lo_d_QA(QA_tag_block):
    # >QA_tag_block< == block of text beginning with one or more tags (== >QA_tag< entries)
    #    followed by one or more QA-text without >QA_tag< entries.
    # Transform >QA_tag_block< into cartesian product (lo of dict): item == every QA_tag with every QA_text.
    ...
    # "SR_TimeStamp": SR_TimeStamp,

    # Get (list of) tag, QA text:
    lo_QA_tag = QA_tag_block["lo_QA_tag"]
    QA_text   = QA_tag_block["QA"]
...
def get_normalized_lo_d_QA__ORG(lo_do_QA, file_path, fn_QA, QA_zotero_hash):
    pass
#     # extract and purge html comments and hashes
#     # normalize Spaced Repetition tag
#
#     lo_do_QA_normalized = []
#     for do_QA in lo_do_QA_SR_raw:
#         s_QA = do_QA["s_QA"]
#
...
#
#         # normalize Spaced Repetition tag:
#         QA_tag     = get_normalized_QA_deck(do_QA["QA_tag"])
#
#         do_QA["QA_tag"] = QA_tag
#         # do_QA["QA_tag"] = get_normalized_QA_deck(do_QA["QA_tag"])
...
def get_normalized_lo_do_QA_SR(lo_do_QA_SR_raw, file_path, QA_zotero_hash):
    # normalize entries (in >lo_do_QA_SR_raw<) ==
    #   add hash if necessary
    #   add color to fn_QA, QA_ID, hash

    lo_do_QA_SR_normalized = []
    for do_QA_SR_raw in lo_do_QA_SR_raw:
        ...
        fn_QA = "<font color=#494429>" + fn_QA + "</font>"

        # colorize strings: QA_ID
        QA_ID = do_QA_SR_raw["QA_ID"]
        QA_ID = "<font color=#494429>" + QA_ID + "</font>"

        # colorize strings: QA_d8_hash
        QA_d8_hash = "<font color=#494429>" + QA_d8_hash + "</font>"
        ...
        lo_do_QA_SR_normalized.append(do_QA_SR_normalized)

    return lo_do_QA_SR_normalized
...
def get_lo_QA_entry(lo_fn_note):
    ...
    for do_QA_note in lo_do_QA_note:
        ...
        # QA_tag_block == block of text beginning with tag indicating tag of one or more QAs.
        lo_QA_tag_block = get_lo_QA_tag_block(content)
        ...
        for QA_tag_block in lo_QA_tag_block:
            lo_do_QA_entry_org.extend(get_lo_d_QA(QA_tag_block))
...
def split_f_SR_into_lo_do_QA_SR(fn_SR) -> list:
    ...
    for txt_block in lo_txt_block:
        lines = txt_block.strip().splitlines()
        # May be no text at all in >txt_block<, only empty lines -> next text block
        ...
        tag_line = lines[0]
        # QA - lines:
        txt_lines = lines[1:]  # text lines up to the end of text block.
        ...
        # Prepare result >lo_do_QA_SR<
        # If there are multiple tags in >tag_line< then
        #   transform textblock in multiple dicts of QA:
        # ToDo : Obiges darf nicht der Fall sein, es darf immer nur ein flashcard-tag vorhanden sein.
        #  Aber nutze das Code Fragment beim Erstellen neuer QA_SRs.
        lo_do_QA_SR_raw = []
        for do_QA_SR_org in lo_do_QA_SR_org:
            lo_do_QA_SR_raw.extend(get_lo_d_QA(do_QA_SR_org))

        # Normalize do_QA_SR: colorize text, adjust value of >QA_d8_hash<):
        if lo_do_QA_SR_raw:
            lo_do_QA_SR.extend(
                get_normalized_lo_do_QA_SR(lo_do_QA_SR_raw,file_path=fn_SR,QA_zotero_hash='flashcar'))
        return lo_do_QA_SR
...
def get_lo_do_QA_SR(lo_fn_SR) -> list:
    ...
    for fn_SR in lo_fn_SR:
        ...
        lo_QA_deck_block = []  # list text blocks, that contain QA
        ...
        for block_text in lo_block_text:
            ...
            # May be no text at all in >block_text<, only empty lines -> next text block
            ...
            # First line contains '#flashcards' tags indicating decks
            deck_line = lines[0]
            # QA - lines:
            txt_lines  = lines[1:] # lines up to the end of text block.
            # >txt_lines< contain Q&A ...
            # followed by QA_separator (global, defined in tac.ini)
            # followed by fn_QA of obsidian note of origin
            # followed by QA_ID
            ...
            fn_QA        = txt_lines[idx_separ + 1],
            QA_ID        = txt_lines[idx_separ + 2],
            SR_TimeStamp = txt_lines[idx_separ + 2],
            ...
            if lo_do_QA_entry_org:
                lo_do_QA_SR.extend(get_normalized_lo_do_QA(lo_do_QA_entry_org, file_path = fn_SR, fn_QA= os.path.basename(fn_SR), QA_zotero_hash ='flashcar'))
    return lo_do_QA_SR
...
def do_p_fn_rename_w_timestamp(lo_p_fn_flashcard, p_QA):
    # >lo_fn_SR< == a list of paths and >dir_QA_cards< a directory path.
    # It renames every element >p_fn_flashcard< of >lo_fn_SR< by adding to the tail of the filename
    ...
    for p_fn in lo_p_fn_flashcard:
        ...
        #     new_path = os.path.join(dir_path or dir_QA_cards, new_fn)
        #     counter += 1
        #     if counter > 60:
        #         break
...
def sort_and_check_lo_do_QA(lo_do_QA_flashcard):
    ...
    lo_do_QA_flashcard_sorted = sorted(lo_do_QA_flashcard, key=lambda x: (x["QA_tag"].lower()))
...
def write_QA_SR_file(lo_do_QA_merged):
    # return
    # Check in directory >dir_QA_cards< if there is a file >fn_QA_SR<.
    ...
    #    use >do_p_fn_rename_w_timestamp(lo_fn_SR, dir_QA_cards)< to rename the existing version.
    #
    ...
    # Compose the full path >p_fn< of QA_SR-file (file with SR flashcards); create it if not existing.
    p_fn = os.path.join(dir_QA_cards, fn_QA_SR).replace('\\', '/')
    # Initialize flashcard file if it does not exist:
    ...
    # Read QA_entries in the existing flashcard file >p_fn<
    # lo_p_fn_SR = [p_fn]
    ...
    if b_modified_lo_do_QA_flashcard:
        do_p_fn_rename_w_timestamp(lo_p_fn_SR, dir_QA_cards)
